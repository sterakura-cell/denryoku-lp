...
test("keeps the winning title while strengthening the answer and diagnosis journey", async () => {
  ...
  assert.match(html, /dateModified": "2026-08-19"/);
  assert.match(html, /更新日：2026年8月19日/);
  assert.match(html, /1日あたり（30日換算）/);
  assert.match(html, /公的な業界平均ではありません/);
  assert.match(html, /実際の1日平均は、対象月の請求額をその月の営業日数で割って確認/);
  assert.match(html, /電力会社を切り替える前に何を確認すればよいですか/);
  assert.match(html, /単月の安さや削減率だけで決めず、年間総額と条件を比較/);
  assert.match(html, /公的な業界統一平均は確認できません/);
  assert.match(html, /休業日や季節変動を反映しないため実績値とは分けて/);
  assert.match(html, /確認できない項目は推測せず供給会社へ照会/);
  assert.doesNotMatch(html, /電力会社を切り替えると営業に影響はありますか/);
  assert.doesNotMatch(html, /停電リスクや電気の品質は従来と変わりません/);
  ...
  assert.match(html, /data-journey="pillar_to_bill_checklist"/);
...
test("updates sitemap dates only for pages changed in this pass", async () => {
  ...
  for (const path of [
    ...
    const expected = path === "pachinko-denkidai.html" ? "2026-08-19" : path === "pachinko-denkidai-takai-riyu.html" ? "2026-08-14" : "2026-07-22";
